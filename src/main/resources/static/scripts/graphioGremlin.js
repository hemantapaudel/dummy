...
//                        }
	...
	function get_graph_info(){
		...
		var message = "";
        run_ajax_request_custom('graphInfo',null,message,null);
	...
	function search_query() {
		...
		var message = "";
        run_ajax_request_custom('search',null,message,null);
		// if (SINGLE_COMMANDS_AND_NO_VARS) {
	...
	function click_query(d) {
        ...
        if (communication_method == "GraphSON3_4") { 
        	...
        	gremlin_query_nodes += '.fold().inject(' + traversal_source + '__.V(' + id + ')).unfold()';
        }
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////
    function run_ajax_request_custom(query_type,active_node,message, callback){
        // while busy, show we're doing something in the messageArea.
        $('#messageArea').html('<h3>(loading)</h3>');
		var baseUrl = 'http://localhost:8080';
		var queryUrl= '';
        if(query_type=="search") queryUrl = "/getHighLevelView";
        else if (query_type=="graphInfo") queryUrl = "/getHighLevelStats";
        ...
        $.ajax({
            ...
            error: function(result, status, error){
                ...
                // output area.
                let msgs = [];
                msgs.push('Problem accessing the custom endpoint using REST at http://localhost:8080/getHighLevelView' );
